...
namespace Org.Reddragonit.Dbpro.Connections
{
	public abstract class Connection : IDataReader
	{
        ...
        public List<Table> Select(string ClassSelectString,List<SelectParameter> parameters)
        {
            if (!ClassSelectString.ToUpper().StartsWith("SELECT FROM "))
            {
                throw new Exception("Unable to process class select function.  Invalid format.");
            }
            System.Reflection.Assembly asm = System.Reflection.Assembly.GetEntryAssembly();
            Type type = asm.GetType(ClassSelectString.Split(" ".ToCharArray())[2],false,false);
            if (type == null)
            {
                throw new Exception("Unable to process class select function.  Unable to locate select class.");
            }
            System.Diagnostics.Debug.Write(ClassSelectString.Replace(ClassSelectString.Split(" ".ToCharArray())[2], ClassMapper.GetTableMap(type).Name));
            return null;
        }
