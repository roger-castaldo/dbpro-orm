...
namespace Org.Reddragonit.Dbpro.Structure.Mapping
{
	internal class TableMap
	{
		...
		public Type GetTypeForField(string ClassName)
		{
			if (_fields.ContainsKey(ClassName))
			{
				if (_fields[ClassName].GetType().Equals(typeof(ExternalFieldMap)))
				{
					return ((ExternalFieldMap)_fields[ClassName]).Type;
				}
			}
			return null;
		}
